        ...
        try {
            ...
            switch (dataType) {
                case 'history':
                    ...
                    filename = `proxy-history-${new Date().toISOString().split('T')[0]}.${format}`;
                    break;
                    
                case 'summary':
                    const summaryResponse = await fetch(`${this.baseUrl}/api/summary`);
                    data = await summaryResponse.json();
                    filename = `proxy-summary-${new Date().toISOString().split('T')[0]}.${format}`;
                    break;
                    
                case 'config':
                    const configResponse = await fetch(`${this.baseUrl}/api/config/get`);
                    data = await configResponse.json();
                    filename = `proxy-config-${new Date().toISOString().split('T')[0]}.${format}`;
                    break;

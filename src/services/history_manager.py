...
class HistoryManager:
    ...
    async def get_recent_messages(self, limit: int = 5) -> MessageHistoryResponse:
        ...
        try:
            ...
            messages = [
                MessageHistoryItem(**msg) for msg in raw_messages
            ]
    ...
    async def get_message_summaries(self, limit: int = 5) -> List[MessageHistorySummary]:
    ...
    async def get_message_by_id(self, message_id: int) -> Optional[MessageHistoryItem]:
        ...
        try:
            messages = await self.database.get_recent_messages(limit=1000)  # Get enough to find by ID
        ...
        for key, value in request_data.items():
            if key == 'messages':
                ...
                for header_key in list(headers.keys()):
                    if 'api' in header_key.lower() or 'auth' in header_key.lower() or 'key' in header_key.lower():
    ...
    async def get_token_usage_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_input_tokens = sum(item['total_input_tokens'] for item in summary_data)
            total_output_tokens = sum(item['total_output_tokens'] for item in summary_data)
